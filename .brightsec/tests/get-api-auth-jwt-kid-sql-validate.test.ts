...
test('GET /api/auth/jwt/kid-sql/validate', { signal: AbortSignal.timeout(timeout) }, async () => {
  await runner
    .createScan({
      tests: ['jwt', 'sqli', 'full_path_disclosure', 'improper_asset_management'],
    ...
    .run({
      ...
      headers: {
        authorization: 'Bearer eyJhbGciOiJIUzI1NiJ9.eyJ1c2VyIjoiamFuZS5kb2VAZXhhbXBsZS5jb20ifQ.signature'
      }
    });
});